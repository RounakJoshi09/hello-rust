...
fn part_1() -> bool {
    ...
    let is_admin = access.filter(|acc| acc == &Access::Admin);

    return is_admin.is_some();
}
...
fn part_2() -> Option<Access> {
    ...
    return access.or_else(|| Some(Access::Admin));
}
...
fn part_3() -> Access {
    ...
    return access.unwrap_or_else(|| Access::Guest);
}
