...
fn main() {
    ...
    stream.for_each(|data| match data {
        [first, second] => println!("First Number : {:?}, Second Number :{:?}", first, second),
        [single] => println!("Unpaired Number : {:?}", single),
        [] => println!("No Number Recieved"),
        [..] => unreachable!("Improper Number Recieved"),
    });
}
